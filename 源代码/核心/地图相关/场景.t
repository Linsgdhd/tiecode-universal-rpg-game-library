/*
	1、如何为不同实体定义不同图层的同时减少循环次数保证性能
		i 在数据更新循环的同时将不同图层的实体添加到封装后的集合内
*/
@强制输出
@输出名("rev2cjcs")
类 场景v1d5
...
类 场景v1d1 : 可绘制实体
	...
	变量 容器 : 世界v1d1?
	变量 相机 : 相机v1d1?
	事件 场景v1d1 : 初始化(data : JSON对象,world : 世界v1d1)
		容器 = world
	结束 事件
	事件 场景v1d1 : 数据更新()
		循环(i, 0, 实体集合.长度)
			变量 k : 可绘制实体 = 实体集合.获取(i)
			如果 k.可访问 则
				k.数据更新()
			结束 如果
		结束 循环
	结束 事件
	事件 场景v1d1 : 绘制(canvas : 画布)
		循环(i, 0, 实体集合.长度)
			变量 k : 可绘制实体 = 实体集合.获取(i)
			如果 k.可访问 则
				k.绘制(canvas)
...
类 位图缓存器
	...
	结束 方法
	方法 取位图(资源名称 : 文本) : 位图对象
		变量 key : 整数 = 索引.寻找成员(资源名称)
		如果 key > 0 且 key <= 容器.长度 则
			变量 temp : 位图对象 = 容器.取成员(key)
			返回 temp
		否则
			返回 空
		结束 如果
...
/*
	1、相机移动时，网格会出现缝隙，问题出在哪里？因为单精度小数被滥用吗
		i 执行绘制时将相机坐标使用.到整数()转换一次  |  可行
	2、使用Choreographer.FrameCallback控制每一帧，帧率受系统调度，手指离开屏幕会导致帧率下降
		i   弄明白自定义绘制线程导致触摸事件出现延时的原因，并不再使用Choreographer.FrameCallback来控制每一帧  |  不想
		ii  android.view.SurfaceView是否被错误使用了？  | 应该是的
		iii 封装并使用GLSurfaceView可以规避以上问题吗?   | 是我异想天开了
*/
类 场景
	变量 场景id : 整数
	变量 场景名称 : 文本
...
类 相机
	...
	事件 目标实体 : 数据更新完成()
		...
		如果 位置.x != xe 且 用户输入 则
			缓冲位置.x = 缓冲位置.x + (xe - 位置.x) * 0.02f
		结束 如果
		如果 位置.y != ye 且 用户输入 则
			缓冲位置.y = 缓冲位置.y + (ye - 位置.y) * 0.02f
		结束 如果
	结束 事件
